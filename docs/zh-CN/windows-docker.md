...
```powershell
& .\install-go.ps1 -Version v0.1.11
```

安装器默认先探测 Gitee，再探测 GitHub。需要指定源时可以执行：

```powershell
& .\install-go.ps1 -Version v0.1.11 -ReleaseSource gitee
```
...
如果检测到已有安装，安装器会让你选择“更新”“重新安装”或“退出”：

- 更新保留现有数据、配置和 Cloud Key；
- 重新安装会删除已经关联的数据源、分配的子账号、数据权限和本地审计
  数据，重装后必须重新配置，并要求输入 `REINSTALL` 确认；
- 重新安装前请先备份需要保留的数据。

DataMind 不会保留或记录用户私有业务数据，请放心使用。数据源配置、元数据、
账号权限和审计信息只保存在您自己的 Docker 部署环境中。详细边界请参阅
